...
def ncRNA_dict(dna_file,gff_file):
    genome_dict = {'Gene_name':[],
                   'Ensemble_id':[],
                   "Locus":[]} 
...
if __name__ == "__main__":
    ...
    df2 = ncRNA_dict(args.dna_file, args.gff_file)
    df2.to_csv(args.output_file.replace('.csv', '_ncRNA.csv'), index=False)
    print(f"ncRNA Lookup table saved to {args.output_file.replace('.csv', '_ncRNA.csv')}")
